...
async function fetchTopMovers({ timeframe, maxPrice, minPrice, condition, printing, rarityFilter, pokemonSet, useSetFilter, setNameFilter, limit, offset }) {
  const params = new URLSearchParams({
    ...
    setNameFilter: String(setNameFilter || ""),
    limit: String(limit),
...
export default function PokemonTopMoversApp() {
  ...
  async function handleFetch() {
    ...
    try {
      ...
      setRows(data);
      if (!data.length) {
        setError("No matching cards found. Raise API Limit, adjust Min/Max Price, clear the Set filters, or choose Any condition/printing.");
      }
    } catch (err) {
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="rounded-3xl border border-slate-800 bg-slate-900/70 p-5 shadow-2xl md:p-6">
          ...
          <div className="grid gap-4 md:grid-cols-4 lg:grid-cols-9">
            ...
            <label className="md:col-span-2">
              <span className="text-sm text-slate-400">Set Name Search</span>
              <input
                value={setNameFilter}
                onChange={(e) => setSetNameFilter(e.target.value)}
                placeholder="ex: Surging Sparks"
                className="mt-1 w-full rounded-2xl border border-slate-700 bg-slate-950 px-4 py-3 text-slate-100 outline-none focus:border-orange-400"
              />
            </label>
